		...
		if (aoeEndPosIndex >= 0)
		{
			using (List<ActorData>.Enumerator enumerator = aoeHitActors.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					...
					actorHitContext.m_contextVars.SetValue(ContextKeys.s_DistFromMin.GetKey(), value);
			...
			using (List<ActorData>.Enumerator enumerator2 = laserHitActorsList[i].GetEnumerator())
			{
				while (enumerator2.MoveNext())
				{
					...
					if (!aoeHitActors.Contains(current2))
					{
						...
						actorHitContext2.m_contextVars.SetValue(ContextKeys.s_DistFromMin.GetKey(), value);
						actorHitContext2.m_contextVars.SetValue(ContextKeys.s_InAoe.GetKey(), 0);
					}
				}
			}
		}
