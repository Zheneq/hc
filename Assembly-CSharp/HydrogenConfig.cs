...
public class HydrogenConfig : JsonConfig
{
    ...
    private string SaveSubTypesPath => Application.dataPath + "/../LocalSettings/SelectedGameSubTypes.json";
    private string LocalSettingsPath => Application.dataPath + "/../LocalSettings/";
    ...
    public override void PostProcess()
    {
        ...
        GameStatsSpreadsheetLogFilePath =
            GameStatsSpreadsheetLogFilePath.SafeReplace("${DATAPATH}", Application.dataPath);
    ...
    public static bool MatchesPath(string path)
    {
        ...
        return fileName.StartsWith("AtlasReactorConfig", StringComparison.OrdinalIgnoreCase)
               || fileName.StartsWith("AtlasReactorDevConfig", StringComparison.OrdinalIgnoreCase)
               || fileName.StartsWith("AtlasReactorServerConfig", StringComparison.OrdinalIgnoreCase);
    }
    ...
    {
        if (AutoLaunchGameType == GameType.Practice || AutoLaunchGameType == GameType.Tutorial)
        {
            return true;
        }
        ...
        {
            return AutoLaunchCustomGameConfig != null
                   && AutoLaunchCustomGameConfig.GameConfig != null
                   && AutoLaunchCustomGameConfig.GameConfig.GameType.IsAutoLaunchable()
                   && !AutoLaunchCustomGameConfig.GameConfig.Map.IsNullOrEmpty();
        }
    ...
    {
        ushort mask = 0;
        string fullPath = Path.GetFullPath(SaveSubTypesPath);
        ...
        try
        {
            ...
            Dictionary<GameType, List<string>> savedGameTypes = new Dictionary<GameType, List<string>>();
            JsonSerializerSettings settings = new JsonSerializerSettings
            {
                ObjectCreationHandling = ObjectCreationHandling.Replace
            };
            JsonConvert.PopulateObject(savedJson, savedGameTypes, settings);
            if (savedGameTypes.TryGetValue(gameType, out var savedSubTypes))
            {
                foreach (KeyValuePair<ushort, GameSubType> current in subTypes)
                {
                    if (savedSubTypes.Contains(current.Value.LocalizedName))
                    {
                        mask = (ushort)(mask | current.Key);
                    }
                }
            }
        ...
        catch (Exception exception)
        {
            ...
            mask = 0;
            SaveGameTypeSubMaskPreference(gameType, mask, ClientGameManager.Get().GameTypeAvailabilies);
            return mask;
        }
    }

    public void SaveGameTypeSubMaskPreference(
        GameType gameType,
        ushort mask,
        Dictionary<GameType, GameTypeAvailability> GameTypeAvailabilies)
    {
        List<string> list = new List<string>();
        if (GameTypeAvailabilies.TryGetValue(gameType, out GameTypeAvailability gameTypeAvailability)
            && !gameTypeAvailability.SubTypes.IsNullOrEmpty())
        {
            ushort subTypeBit = 1;
            foreach (GameSubType subType in gameTypeAvailability.SubTypes)
            {
                if ((subTypeBit & mask) != 0)
                {
                    list.Add(subType.LocalizedName);
                }

                subTypeBit = (ushort)(subTypeBit << 1);
            }
        }
        ...
        {
            string fullPath = Path.GetFullPath(SaveSubTypesPath);
            Dictionary<GameType, List<string>> gameTypesToSave = new Dictionary<GameType, List<string>>();
            ...
            if (fileInfo.Exists)
            {
                ...
                JsonConvert.PopulateObject(text, gameTypesToSave, settings);
            }
            else
            {
