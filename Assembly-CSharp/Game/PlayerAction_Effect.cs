...
public class PlayerAction_Effect
{
	...
	public PlayerAction_Effect(List<Effect> requests, AbilityPriority phase)
	{
		m_requests = requests.SelectMany(delegate (Effect e)
		{
			// rogues
	...
	public List<ActorAnimation> PrepareResults()
	{
		if (m_requests != null)
		{
			...
			List<ActorAnimation> list = new List<ActorAnimation>();
			m_requests = (from effectResults in m_requests
							   where effectResults.m_actorToHitResults.Any() || effectResults.m_positionToHitResults.Any() || effectResults.m_sequenceStartData.Any()
							   select effectResults).ToList();
			sbyte b = 0;
			foreach (EffectResults effectResults2 in m_requests)
			{
	...
	{
		foreach (EffectResults effectResults in m_requests)
		{
			// rogues
