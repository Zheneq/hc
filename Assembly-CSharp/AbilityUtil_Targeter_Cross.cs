...
public class AbilityUtil_Targeter_Cross : AbilityUtil_Targeter
{
	...
	public override void UpdateTargeting(AbilityTarget currentTarget, ActorData targetingActor)
	{
		...
		{
			if (targetingActor.GetCurrentBoardSquare().symbol_0013(boardSquare.x, boardSquare.y))
			{
				bool flag;
				Vector3 vector4;
	...
	private void DrawInvalidSquareIndicators(AbilityTarget currentTarget, ActorData targetingActor, Vector3 startPos, Vector3 endPos, Vector3 crossStart, Vector3 crossEnd)
	{
		if (targetingActor == GameFlowData.Get().activeOwnedActorData)
		{
			...
			if (boardSquare != null)
			{
				if (boardSquare.height <= Board.Get().BaselineHeight && targetingActor.GetCurrentBoardSquare().symbol_0013(boardSquare.x, boardSquare.y))
				{
