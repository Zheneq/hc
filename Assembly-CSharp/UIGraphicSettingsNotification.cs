...
public class UIGraphicSettingsNotification : MonoBehaviour
{
    ...
    public static void SetVisible(bool visible, CloseCallback callback)
    {
        s_isVisible = visible;
        m_callback = callback;
        if (visible && s_instance == null)
        {
            Log.Warning(
                "Called to display Graphic Settings Notification Visible while HUD has not been loaded in yet");
        }
    }
    ...
    private void Update()
    {
        if (m_graphicSettingsContainer != null
            && m_graphicSettingsContainer.gameObject.activeSelf != s_isVisible)
        {
