...
public class ValidatingActionRequestState : TurnState
{
	...
	public override void OnMsg(TurnMessage msg, int extraData)
	{
		...
		case TurnMessage.MOVEMENT_RESOLVED:
			Log.Error(m_SM.GetComponent<ActorData>().DisplayName + "Received a 'Movement Resolved' message in the ValidatingActionRequest state, which is unexpected.");
			m_SM.NextState = TurnStateEnum.WAITING;
			break;
		case TurnMessage.CLIENTS_RESOLVED_ABILITIES:
			if (NetworkServer.active)
			{
				Log.Error(m_SM.GetComponent<ActorData>().DisplayName + "Received a 'CLIENTS_RESOLVED_ABILITIES' message in the ValidatingActionRequest state, which is unexpected.");
			}
			else
			{
				Log.Warning(m_SM.GetComponent<ActorData>().DisplayName + "Received a 'CLIENTS_RESOLVED_ABILITIES' message in the ValidatingActionRequest state, which is unexpected.");
			}
			m_SM.NextState = TurnStateEnum.WAITING;
			break;
		}
