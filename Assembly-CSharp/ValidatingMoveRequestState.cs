...
public class ValidatingMoveRequestState : TurnState
{
	...
	public override void OnMsg(TurnMessage msg, int extraData)
	{
		...
		case TurnMessage.MOVEMENT_RESOLVED:
			Log.Error(new StringBuilder().Append(m_SM.GetComponent<ActorData>().DisplayName).Append("Received a 'Movement Resolved' message in the ValidatingMoveRequest state, which is unexpected.").ToString());
			m_SM.NextState = TurnStateEnum.WAITING;
			break;
		case TurnMessage.CLIENTS_RESOLVED_ABILITIES:
			if (NetworkServer.active)
			{
				Log.Error(new StringBuilder().Append(m_SM.GetComponent<ActorData>().DisplayName).Append("Received a 'CLIENTS_RESOLVED_ABILITIES' message in the ValidatingMoveRequest state, which is unexpected.").ToString());
			}
			else
			{
				Log.Warning(new StringBuilder().Append(m_SM.GetComponent<ActorData>().DisplayName).Append("Received a 'CLIENTS_RESOLVED_ABILITIES' message in the ValidatingMoveRequest state, which is unexpected.").ToString());
			}
			m_SM.NextState = TurnStateEnum.WAITING;
			break;
		}
