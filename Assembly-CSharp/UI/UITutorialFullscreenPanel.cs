...
public class UITutorialFullscreenPanel : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        AnimatorStateInfo currentAnimatorStateInfo =
            m_AnimatorObject.GetComponent<Animator>().GetCurrentAnimatorStateInfo(0);
        if (currentAnimatorStateInfo.IsName("TutorialCombatPhasePanelDONE")
            || currentAnimatorStateInfo.IsName("TutorialPrepPhasePanelDONE")
            || currentAnimatorStateInfo.IsName("TutorialDashPhasePanelDONE")
            || currentAnimatorStateInfo.IsName("TutorialStatusEffectPanelDONE")
            || currentAnimatorStateInfo.IsName("TutorialTeammateTargetingPanelDONE")
            || currentAnimatorStateInfo.IsName("TutorialEnergyAndUltimatesPanelDONE"))
        {
            ClearAllPanels();
        }
    }
    ...
    {
        if (moviePlayer == null || movieName.IsNullOrEmpty())
        {
            return;
        }
        ...
        if (HUD_UIResources.Get().m_prologueVideoList.Length <= movieId)
        {
            return;
        }

        HUD_UIResources.TutorialVideoInfo videoInfo = HUD_UIResources.Get().m_prologueVideoList[movieId];
        string localizedVideoPath = HUD_UIResources.Get().GetLocalizedVideoPath(
            videoInfo,
    ...
    public void ShowCombatPhasePanel()
    {
        ...
        PlayRawImageMovieTexture movieTextures =
            m_combatPhasePanel.GetComponentInChildren<PlayRawImageMovieTexture>(true);
        SetupMoviePlayer(movieTextures, "Combat");
    }
    ...
    public void ShowDashPhasePanel()
    {
        ...
        PlayRawImageMovieTexture movieTextures =
            m_dashPhasePanel.GetComponentInChildren<PlayRawImageMovieTexture>(true);
        SetupMoviePlayer(movieTextures, "Dash");
    }
    ...
    public void ShowPrepPhasePanel()
    {
        ...
        PlayRawImageMovieTexture movieTextures =
            m_prepPhasePanel.GetComponentInChildren<PlayRawImageMovieTexture>(true);
        SetupMoviePlayer(movieTextures, "Prep");
    }
    ...
    public void ClearAllPanels()
    {
        ...
        Animator animator = m_AnimatorObject.GetComponent<Animator>();
        if (animator.isActiveAndEnabled)
        {
            animator.Play("WaitingToRun", 0);
        }
    }
